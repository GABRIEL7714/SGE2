document.addEventListener("DOMContentLoaded", async function () {
  try {
    ...
    <td class="d-flex gap-2">
      <button class="btn btn-primary btn-sm" onclick="materialesEvento('${idEvento}')">Entrega Materiales</button>
      <button class="btn btn-success btn-sm" onclick="tomarAsistencia('${idEvento}')">Tomar Asistencia</button>
    </td>
  `;
...
function tomarAsistencia(idEvento) {
  window.location.href = `/AsistenciaEvento?id=${idEvento}`;
}

function materialesEvento(idEvento) {
  window.location.href = `/MaterialesEvento?id=${idEvento}`;
}
